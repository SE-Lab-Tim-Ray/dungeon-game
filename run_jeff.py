...
CREATE_WALL_INTERVAL = 15  * ONE_SECOND # create new wall block every x seconds
...
LEADER_LOSE_TEXT = "Eaten by rats: "
LOSING_SCORE = -1000
            ...
            while not restart:
                if game_over and key1_count == 1 and key2_count == 1 and key3_count == 1 and key4_count == 1:
                    ...
                    for m in pygame.event.get():
                        if m.type == QUIT:
                            exit()
                        if m.type == KEYDOWN and show_leaderboard:
                            # Restart game
                            key1_count = 0
                            key2_count = 0
                            key3_count = 0
                ...
                if lose_game:
                    if lose_time == 0:
                        ...
                        self.screen.blit(self.maze_lose_img, (0, 0))  # lose bg
                        self.screen.blit(self.font.render(self.nickname, True, WHITE), LOSE_LOCATION)
                        self.screen.blit(self.font.render(TIMER_TEXT + str(counter), True, WHITE), (LOSE_LOCATION[0], LOSE_LOCATION[1]+ NICKNAME_LEADING))

                    now = pygame.time.get_ticks()
                    if lose_time > 0 and now > (lose_time + MAZE_WIN_HOLDSCREEN):
                        show_leaderboard = True
                        # position of each line of the leaderboard
                        # show the leaderboard background
                        self.screen.blit(self.maze_leaderboard_img, (0, 0))
                        ...
                        for i in leader_board_list_of_strings:
                            ...
                            y += LEADER_BOARD_LEADING
                        # put last score below in case its not on the leaderboard
                        last_score_txt = self.font.render(LEADER_LOSE_TEXT + self.nickname + ", " + text_clock, True, WHITE)
                        last_score_txt_rect = last_score_txt.get_rect(center=(SCREEN_SIZE[0]/2, LEADER_BOARD_LAST_SCORE_LOCATION_Y))
                        self.screen.blit(last_score_txt, last_score_txt_rect)
                    pygame.display.update()
                    self.clock.tick(self.fps)
                    ...
                    for m in pygame.event.get():
                        ...
                        self.clock.tick(self.fps)
                        continue
                    # get the result and write into txt file first time through only
                    if(count == 0):
                        # leader_board_content = LeaderBoard.board_input_result(self.nickname, int(counter))
                        leader_board_list_of_strings = LeaderBoard.board_input_result(self.nickname, int(LOSING_SCORE))
                        count += 1
                ...
                ticks = pygame.time.get_ticks()
                if (ticks > last_time + ONE_SECOND) and not lose_game:
                    last_time = ticks
                    counter -= 1

...
class Hero:
    def __init__(self, hero_img, hero_rect, mx, my):
    ...
    def hero_moving(self, press_keys):
        ...
        if press_keys[K_LEFT]:
            ...
            for n in range(len(block_group)-1):     # block_group list is all block, last one is end block
                ...
                if self.hero_rect.top > block_group[n].rect.top - tile_width and self.hero_rect.bottom < block_group[n].rect.bottom + tile_height:
        ...
        if press_keys[K_RIGHT]:
            ...
            self.hero_rect.left += self.hero_speed
            if self.hero_rect.right >= WINDOW_WIDTH:
                self.hero_rect.right = WINDOW_HEIGHT
        ...
        if press_keys[K_UP]:
            ...
            for n in range(len(block_group)-1):
                if self.hero_rect.left > block_group[n].rect.left - tile_width and self.hero_rect.right < block_group[n].rect.right + tile_height:
        ...
        if press_keys[K_DOWN]:
            ...
            self.hero_rect.top += self.hero_speed
            if self.hero_rect.bottom >= WINDOW_WIDTH:
                self.hero_rect.bottom = WINDOW_HEIGHT
            for n in range(len(block_group)-1):
                if self.hero_rect.left > block_group[n].rect.left - tile_width and self.hero_rect.right < block_group[n].rect.right + tile_height:
